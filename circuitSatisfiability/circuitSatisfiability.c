...
int main (int argc, char *argv[]) {
   ...
   for (i = id; i <= UINT_MAX; i += numProcesses) {
      temp_count += checkCircuit (id, i); 
   }
   totalTime = MPI_Wtime() - startTime;
   printf ("Process %d finished in time %f seconds. #Solutions: %d\n", id, totalTime, temp_count);
   ...
   totalTime = MPI_Wtime() - startTime;
   if ( id == 0) {
   printf("\nA total of %d solutions were found in %f seconds.\n\n", count, totalTime);
   }
   return 0;
}
